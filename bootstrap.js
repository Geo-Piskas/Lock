...
function lock(window) {
  ...
  } while( CryptoJS.SHA3(password.value+salt) != passphrase || result==false );
...
let optionObserver = {
	...
	var salt = prefs.getCharPref("salt");
        ...
        if(result==true && CryptoJS.SHA3(password0.value+salt) == passphrase){ 
            ...
            if(result==true){ 
                ...
                if(result==true && password1.value==password2.value){
                    prefs.setCharPref("passphrase",CryptoJS.SHA3(password2.value + salt) );
    ...
    do {
        ...
        if(result==true){
            ...
            if(result==true && password.value==password1.value){
                prefs.setCharPref("passphrase",CryptoJS.SHA3(password.value + salt) );
                ok=true;
            }
        }
    }while(ok==false);
